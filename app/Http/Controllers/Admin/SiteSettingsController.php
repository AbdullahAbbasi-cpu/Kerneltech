...
class SiteSettingsController extends Controller
{
    ...
    public function update(UpdateSiteSettingRequest $request, $id)
    {
        ...
        if ($request->hasfile('main_logo') || isset($records->previous_main_logo)) {
            ...
            $data['main_logo'] = $convertedImage->basename;
            $mainLogoFilename = $convertedImage->basename;
            if (
                file_exists(uploadsDir('site-settings') . $filename)
        ...
        if ($request->hasfile('sticky_logo')) {
            ...
            $extension = $file->getClientOriginalExtension();
            $filename  = 'site-settings\sticky-logo-' . time() . '.' . 'webp';
            // $file->move(uploadsDir('site-settings'), $filename);
            $convertedImage = convertImage($file, $filename);
            $data['sticky_logo'] = $convertedImage->basename;
            $stickyLogoFilename = $convertedImage->basename;
